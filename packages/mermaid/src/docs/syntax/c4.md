...
```mermaid-example
    C4Context
      title 网络银行-系统上下文图
      Enterprise_Boundary(b0, "银行边界0") {
        Person(customerA, "银行客户A", "一个银行客户，拥有个人银行账户")
        Person(customerB, "银行客户B")
        Person_Ext(customerC, "银行客户C", "描述")

        Person(customerD, "银行客户D", "一个银行客户，<br/>拥有个人银行账户")

        System(SystemAA, "网络银行系统", "允许客户查看他们的银行账户信息，并进行支付。")

        Enterprise_Boundary(b1, "银行边界") {

          SystemDb_Ext(SystemE, "大型银行系统", "存储有关客户、账户、交易等的所有核心银行信息。")

          System_Boundary(b2, "银行边界2") {
            System(SystemA, "银行系统A")
            System(SystemB, "银行系统B", "一种银行系统，有个人银行账户。<br/>下一行。")
          }

          System_Ext(SystemC, "电子邮件系统", "内部Microsoft Exchange电子邮件系统。")
          SystemDb(SystemD, "银行系统D-数据库", "银行系统，有个人银行账户。")

          Boundary(b3, "银行边界3", "边界") {
            SystemQueue(SystemF, "银行系统F-队列", "银行中的一个系统")
            SystemQueue_Ext(SystemG, "银行系统G-队列", "银行系统，有个人银行账户。")
          }
        }
      }
